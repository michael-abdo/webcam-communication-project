...
class BaselineCapture {
    ...
    async uploadFaceBaseline(faceBlob) {
        try {
            ...
            if (!this.uploadUrls || !this.uploadUrls.face_upload_url) {
                throw new Error('No face upload URL available');
            }
            
            const uploadUrl = this.uploadUrls.face_upload_url;
            const storageType = this.uploadUrls.storage_type || 'S3';
    ...
    async uploadSpeechBaseline(speechBlob) {
        try {
            ...
            // Get upload URL from session data
            if (!this.uploadUrls || !this.uploadUrls.speech_upload_url) {
                throw new Error('No speech upload URL available');
            }
            
            const uploadUrl = this.uploadUrls.speech_upload_url;
            const storageType = this.uploadUrls.storage_type || 'S3';
    ...
    async _notifyBackendUpload(type) {
        try {
            ...
            const objectKey = type === 'face' ? this.uploadUrls.face_key : this.uploadUrls.speech_key;
            
            const payload = {
                session_id: this.sessionId,
                object_key: objectKey,
                bucket: this.uploadUrls.bucket,
                duration: type === 'face' ? 10 : 15
            };
